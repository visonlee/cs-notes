...
答案: 当`length` 等于0时, `0 - 1` 将执行`无符号`运算，结果得到`UMax`。这样`i <= length - 1`,发生隐式转换,结果永远为真。

改正方法有两种: 1: 把`length`参数类型改为`int`, 2:或者 把for循环测试条件为 i < length
...
答案:
当 `strlen(s) - strlen(t)` 的差为负数时,会变成一个很大的无符号数, 这会导致问题。
改正办法: `return strlen(s) > strlen(t);`


## 浮点数

我们在有限的比特位(16/32/64)表示一个数，是不能够表示所有的实数的。
...
IEEE的标准, 定义了两个基本的格式:32位单精度(float)和64位双精度(doube)

单精度与双精度原理差不多, 这里以单精度为例,单精度的32比特可以分成三部分。

| s(符号位) | e(指数位) | f(有效位数) |
| :----: | :----: | :----: |
| 1 bit | 8 bit | 23 bit |

第一部分: **符号位(s)** 用来表示正数还是负数, 不像整数那样分有符号还是无符号, 所有的浮点数都是有符号的

第二部分: **指数位(e)** 在这里用 1～254 映射到 -126～127 这 254 个有正有负的数上。因为我们的浮点数，不仅仅想要表示很大的数，还希望能够表示很小的数，所以指数位也会有负数。
我们没有用到 0 和 255。没错，这里的 0（也就是 8 个比特全部为 0） 和 255 （也就是 8 个比特全部为 1）另有它用。

第三部分: **有效位数(f)**  综合科学计数法，我们的浮点数就可以表示成下面这样：

> $(-1)^s$ × 1.f × $2^e$


这里的浮点数，没有办法表示 0。要表示 0 和一些特殊的数，我们就要用上在 e 里面留下的 0 和 255 这两个表示，这两个表示其实是两个标记位。·在 e 为 0 且 f 为 0 的时候，我们就把这个浮点数认为是 0。至于其它的 e 是 0 或者 255 的特殊情况，参考下面这个表格，分别可以表示出无穷大、无穷小、NAN 以及一个特殊的不规范数。
...
我们以 0.5 为例子。0.5 的符号为 s 是 0，f 是 0，而 e 是 -1，也就是
